...
void __attribute__((interrupt)) debug_handler() {
...
void __attribute__((naked)) __attribute__((aligned(4))) handler_dispatch() {
    ...
    asm volatile(".align 2; j default_handler"); // 12
    asm volatile(".align 2; j default_handler"); // 13
    asm volatile(".align 2; j default_handler"); // 14
    asm volatile(".align 2; j default_handler"); // 15
    asm volatile(".align 2; j debug_handler"); // 10
}
